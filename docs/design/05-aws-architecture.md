...
上記金額は現時点の設計目標であり、AWSの料金上限を保証するものではない。
...
通常とは異なる急激なコスト増加を検知するため、AWS Cost Anomaly
Detectionを利用する。

深夜等、管理者が即時対応できない時間帯も考慮し、金額到達だけでなく異常な増加を緊急停止判断へ利用する。

### 16.4 緊急停止

異常なコスト増加または予算超過が発生した場合、可用性よりコスト抑制を優先し、課金リソースを可能な範囲で停止する。
...
料金情報・異常検知には反映遅延が存在し得るため、12,000円を厳密なハードキャップとはみなさず、余裕を持った停止条件を設定する。

### 16.5 Spend limits

AWSアカウントでSpend
limitsが利用可能な場合は、AWS標準機能による支出制御を優先して検討する。

利用可否および適用可能範囲はAWSアカウント作成後に確認する。

### 16.6 Pricing Calculator

AWSアカウント作成・リソース構築前にAWS Pricing
Calculatorを利用して、構成全体の月額料金を見積もる。
...
通常アクセス時の総額が許容予算を超える場合は、AWS構築前に構成を見直す。

見積結果は可能であればPDFまたはCSVとして保存し、設計・運用資料として管理する。
